...
import { Position, Mines, PerimeterCell, Difficulty } from 'src/model'
import { deployMines } from 'src/actions/deployMines'


export function minesReducer (state: Mines, action) : Mines {
    return _minesReducer(action.difficulty)(state, action)
}

const _minesReducer = (difficulty: Difficulty) => createReducer(initialState.mines, 
    on(deployMines, state => {
